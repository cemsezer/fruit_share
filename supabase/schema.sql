...
create table if not exists public.listings (
  ...
  pickup_area text not null default '' check (char_length(pickup_area) between 2 and 160),
  location_lat double precision,
  location_lng double precision,
  status text not null default 'active' check (status in ('active', 'reserved', 'collected', 'expired', 'flagged', 'removed')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint valid_window check (available_until is null or available_until > available_from),
  constraint location_pair check ((location_lat is null and location_lng is null) or (location_lat is not null and location_lng is not null))
);
